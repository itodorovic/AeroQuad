...
- The timer counts clock ticks derived from the CPU clock. Using 16MHz CPU clock
  and a prescaler of 8 gives a timer clock of 2MHz, one tick every 0.5s. This
...
In the code below, the periode shall be 20ms, so the ICRx register is set to 
 40000 ticks of 0.5s/tick. It probably should be 39999, but who cares about
 this 0.5s for the periode.
The high time shall be 1500s, so the OCRxy register is set to 3000. A change of
 the timer periode does not change this setting, as the clock rate is still one
 tick every 0.5s. If the prescaler was changed, the OCRxy register value would
 be different. 
*/
